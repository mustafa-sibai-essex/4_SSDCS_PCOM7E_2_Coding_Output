...
def vulnerability(request, vuln_id):
    ...
    return render(
        request, "operatorview/vulnerability.html", {"vulnerability": vulnerability}
    )
...
def Awaiting(request):
    ...
    return render(
        request,
        "operatorview/awaiting.html",
        {"vulnerabilities": Vulnerabilities.objects.filter(status="Awaiting Approval")},
    )
    ...
    if request.method == "GET":
        ...
        try:
            ...
            vulnerabilities = Vulnerabilities.objects.filter(
                Q(status=query) | Q(assigned_to=query)
            )
    return render(
        request,
        "operatorview/searchresults.html",
        {"query": query, "vulnerabilities": vulnerabilities},
    )
